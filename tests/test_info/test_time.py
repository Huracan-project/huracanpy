...
def test_time_components(tracks_csv):
    ...
    np.testing.assert_equal(year, np.asarray([1980] * len(tracks_csv.time)))
    np.testing.assert_equal(month, np.asarray([1] * len(tracks_csv.time)))

    days = np.concatenate(
        [
            # First track
            np.asarray([6] * 3),
            np.concatenate([np.asarray([n] * 4) for n in range(7, 13 + 1)]),
            # Second track
            np.concatenate([np.asarray([n] * 4) for n in range(7, 8 + 1)]),
            np.asarray([9] * 2),
            np.concatenate([np.asarray([n] * 4) for n in range(10, 11 + 1)]),
            np.asarray([12] * 2),
            # Third track
            np.asarray([17, 17, 18, 19, 19, 20, 20, 20]),
            np.concatenate([np.asarray([n] * 4) for n in range(21, 30 + 1)]),
        ]
    )
    ...
    hours = np.concatenate(
        [
            # First track
            np.asarray([6, 12, 18, 0] * 8)[:-1],
            # Second track
            np.asarray([0, 6, 12, 18] * 2),
            np.asarray([6, 18]),
            np.asarray([0, 6, 12, 18] * 3)[:-2],
            # Third track
            np.asarray([6, 18, 6, 0, 6, 6, 12, 18]),
            np.asarray([0, 6, 12, 18] * 10),
        ]
    )
