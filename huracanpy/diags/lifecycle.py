...
def time_from_genesis(data):
    ...
    data_df = data_df.merge(data_df.groupby("track_id").time.min(), on = "track_id", suffixes = ["_actual", "_gen"])
    time_from_start = data_df.time_actual - data_df.time_gen
    return time_from_start.to_xarray().rename({"index":"obs"}).rename("time_from_genesis")
...
def time_from_extremum(data, varname, stat = max):
    ...
    return time_from_extr.to_xarray().rename({"index":"obs"}).rename("time_from_extremum")