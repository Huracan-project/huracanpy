...
def identify_line_type(content):
    if len(content) == 28:
        ...
        print("Line type not recognized")
        return None
...
def load(filename):
    ...
    # Format
    df = pd.DataFrame(L, columns = ["track_id", "time", "lat", "lon", "wind", "pres", "lat_wind", "lon_wind"])
