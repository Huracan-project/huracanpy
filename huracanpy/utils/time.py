...
def get_season(track_id, lat, time, convention = "long"): 
    ...
    if convention == "short" :
        ...
        season = np.where((group.hemi == "S") & (group.month >= 7), group.year + 1, season)
        ...
        season = np.where((group.hemi == "S") & (group.month >= 7), group.year.astype(str) + (group.year + 1).astype(str), season)
        season = np.where((group.hemi == "S") & (group.month <= 6), (group.year - 1).astype(str) + group.year.astype(str), season)
    else : 
